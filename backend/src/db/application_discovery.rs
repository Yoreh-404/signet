...
use super::{
    ...
    ApplicationDiscoveryLease, ApplicationDiscoveryRecord, ClientRecord, CountRow, DatabaseKind,
    Db, NewApplicationDiscovery, WebsiteDiscoveryConnection, WebsiteDiscoveryProfileInput,
        ...
        with_conn!(self, |conn, kind| {
            conn.transaction::<ApplicationDiscoveryRecord, AppError, _>(|conn| {
                ...
                if let Some(default_profile_id) = profile_db_ids.get("default") {
                    ...
                    let profile_placeholders = placeholders(kind, 1, profile_ids.len());
                    for table in [
                        "application_profile_group_roles",
                        "application_profile_organization_roles",
                    ] {
                        let delete_sql = format!(
                            "DELETE FROM {table} WHERE profile_id IN ({profile_placeholders})"
                        );
                        bind_text_list(conn, sql_query(delete_sql), &profile_ids)
                            .execute(conn)
                            .map_err(AppError::from)?;
                    }
