...
Signet 提供可直接使用的管理控制台，覆盖账号生命周期、OIDC 客户端、身份源、组织、安全策略和审计；登录端支持密码、Passkey、MFA 与企业身份源。
...
| 身份与组织 | 管理用户生命周期、组织成员和角色；按组织隔离 OIDC 应用。 |
| 应用接入 | 标准 OIDC discovery、授权码与 PKCE、refresh token、客户端认证和授权同意。 |
| 企业集成 | 外部 OIDC、LDAP/AD、SCIM 2.0、IAP/ForwardAuth、审计 Webhook 和服务账号。 |
